...
package org.foodpanda.gui.password;
...
public class ChangePasswordView extends AbstractFxController implements DomainContextProvider {
  ...
  private void save() {
    if (admin || Objects.equals(foodpandaHelper.hash(oldPassword), oldPasswordHash)) {
      if (!Arrays.equals(newPassword, confirmedNewPassword)) {
        ...
        if (newPassword == null) {
          ...
          Fx.yes(resources.getString("save new password?"), false, () -> saveAndClose(foodpandaHelper.hash(newPassword)));
        }
      }
    }
    else {
